...
vim.keymap.set('t', '<C-Esc>', '<C-\\><C-n>', { desc = 'Exit terminal mode with Ctrl+Esc' })
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sw', function()
        ...
        while i <= #output do
          if output[i]:match '^worktree ' then
            local path = output[i]:gsub('^worktree ', '')
            local branch = ''
            local is_bare = false
            -- Look ahead for branch info
            while i + 1 <= #output and not output[i + 1]:match '^worktree ' do
              i = i + 1
              if output[i]:match '^branch ' then
                branch = output[i]:gsub('^branch refs/heads/', '')
              elseif output[i]:match '^bare' then
                is_bare = true
              end
            end
            if not is_bare then
            ...
            finder = finders.new_table {
              ...
              entry_maker = function(entry)
                ...
                return {
                  value = entry,
                  display = display,
                  ordinal = entry.display .. ' ' .. entry.branch,
                }
              end,
            },
      ...
      vim.keymap.set('n', '<leader>sm', builtin.man_pages, { desc = '[S]earch by [G]rep' })
      vim.keymap.set('n', '<leader>sd', builtin.diagnostics, { desc = '[S]earch [D]iagnostics' })
      vim.keymap.set('n', '<leader>sc', builtin.commands, { desc = '[S]earch [C]ommands' })
      vim.keymap.set('n', '<leader>sr', builtin.resume, { desc = '[S]earch [R]esume' })
      vim.keymap.set('n', '<leader>st', builtin.git_status, { desc = 'Git [S][t]atus' })
  ...
  { import = 'custom.plugins', vscode = true },
}, {
  defaults = {
